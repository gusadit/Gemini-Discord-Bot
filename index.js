    ...
    try {
        ...
        if (shouldRespond) {
            if (message.guild) {
                ...
                if (
                    state.blacklistedUsers[message.guild.id].includes(
                        message.author.id
                    )
                ) {
...
async function handleButtonInteraction(interaction) {
    ...
    if (interaction.guild) {
        ...
        if (
            state.blacklistedUsers[interaction.guild.id].includes(
                interaction.user.id
            )
        ) {
            const embed = new EmbedBuilder()
                ...
                .setTitle("Blacklisted")
                .setDescription(
                    "You are blacklisted and cannot use this interaction."
                );
    ...
    // Cek tombol Settings: hanya admin
    if (interaction.customId === "settings") {
        if (
            ...
            const embed = new EmbedBuilder()
                ...
                .setDescription("⚠️ Only admins can access Settings.");
            return interaction.reply({
                embeds: [embed],
                flags: MessageFlags.Ephemeral,
            });
...
async function handleToggleResponseMode(interaction) {
    const serverResponsePreferenceEnabled = interaction.guild
        ? state.serverSettings[interaction.guild.id]?.serverResponsePreference
        : false;
...
async function handleTextMessage(message) {
    ...
    try {
        if (SEND_RETRY_ERRORS_TO_DISCORD) {
            ...
            parts = await processPromptAndMediaAttachments(
                messageContent,
                message
            );
    ...
    if (guildId) {
        if (state.channelWideChatHistory[channelId]) {
            ...
            state.serverSettings[guildId]?.customServerPersonality &&
            state.customInstructions[guildId]
        ) {
    ...
    const isServerChatHistoryEnabled = guildId
        ? state.serverSettings[guildId]?.serverChatHistory
        : false;
    const isChannelChatHistoryEnabled = guildId
        ? state.channelWideChatHistory[channelId]
        : false;
...
async function processPromptAndMediaAttachments(prompt, message) {
    ...
    if (attachments.length > 0) {
        ...
        if (validAttachments.length > 0) {
            const attachmentParts = await Promise.all(
                validAttachments.map(async (attachment) => {
                    ...
                    try {
                        ...
                        if (name === null) {
                            throw new Error(
                                `Unable to extract file name from upload result.`
                            );
                        }
            ...
            parts = [
                ...parts,
                ...attachmentParts.filter((part) => part !== null),
            ];
        }
    }
    return parts;
    ...
    if (message.attachments.size > 0) {
        ...
        for (const attachment of attachments) {
            ...
            if (fileTypes.includes(fileType)) {
                try {
                    let fileContent = await downloadAndReadFile(
                        attachment.url,
                        fileType
                    );
...
async function handleModalSubmit(interaction) {
    if (interaction.customId === "custom-personality-modal") {
        try {
            ...
            state.customInstructions[interaction.user.id] =
                customInstructionsInput.trim();
        ...
        try {
            ...
            state.customInstructions[interaction.guild.id] =
                customInstructionsInput.trim();
            ...
            const embed = new EmbedBuilder()
                ...
                .setTitle("Success")
                .setDescription(
                    "Custom Server Personality Instructions Saved!"
                );
            await interaction.reply({
                embeds: [embed],
                flags: MessageFlags.Ephemeral,
...
async function handleStatusCommand(interaction) {
    ...
    if (
        ...
        const embed = new EmbedBuilder()
            .setColor(0xff0000)
            .setTitle("Access Denied")
            .setDescription("⚠️ Only admins can use this command.");
        return interaction.reply({
            embeds: [embed],
            flags: MessageFlags.Ephemeral,
        });
...
async function downloadMessage(interaction) {
    try {
        ...
        const filePath = path.join(
            TEMP_DIR,
            `message_content_${interaction.id}.txt`
        );
        ...
        if (interaction.channel.type === ChannelType.DM) {
            ...
            try {
                ...
                const dmSentEmbed = new EmbedBuilder()
                    ...
                    .setDescription(
                        "The message content has been sent to your DMs."
                    );
                await interaction.reply({
                    embeds: [dmSentEmbed],
                    flags: MessageFlags.Ephemeral,
...
async function downloadConversation(interaction) {
    try {
        ...
        const tempFileName = path.join(
            TEMP_DIR,
            `conversation_${interaction.id}.txt`
        );
        ...
        try {
            if (interaction.channel.type === ChannelType.DM) {
                ...
                const dmSentEmbed = new EmbedBuilder()
                    ...
                    .setDescription(
                        "Your conversation history has been sent to your DMs."
                    );
                await interaction.reply({
                    embeds: [dmSentEmbed],
                    flags: MessageFlags.Ephemeral,
            ...
            const failDMEmbed = new EmbedBuilder()
                ...
                .setTitle("Delivery Failed")
                .setDescription(
                    "Failed to send the conversation history to your DMs."
                );
            await interaction.reply({
                embeds: [failDMEmbed],
                files: [file],
...
async function toggleUserResponsePreference(interaction) {
    try {
        ...
        state.userResponsePreference[userId] =
...
async function toggleToolPreference(interaction) {
    try {
        ...
        const options = [
            "Google Search with URL Context",
            "Code Execution",
            "Function Calling",
        ];
...
async function toggleServerWideChatHistory(interaction) {
    try {
        ...
        state.serverSettings[serverId].serverChatHistory =
            !state.serverSettings[serverId].serverChatHistory;
        const statusMessage = `Server-wide Chat History is now \`${
            state.serverSettings[serverId].serverChatHistory
                ? "enabled"
                : "disabled"
        }\``;
        ...
        if (
            state.serverSettings[serverId].serverChatHistory &&
            !state.serverSettings[serverId].customServerPersonality
        ) {
            warningMessage =
...
async function toggleServerPersonality(interaction) {
    try {
        ...
        state.serverSettings[serverId].customServerPersonality =
            !state.serverSettings[serverId].customServerPersonality;
        const statusMessage = `Server-wide Personality is now \`${
            state.serverSettings[serverId].customServerPersonality
                ? "enabled"
                : "disabled"
        }\``;
...
async function toggleServerResponsePreference(interaction) {
    try {
        ...
        state.serverSettings[serverId].serverResponsePreference =
            !state.serverSettings[serverId].serverResponsePreference;
        const statusMessage = `Server-wide Response Following is now \`${
            state.serverSettings[serverId].serverResponsePreference
                ? "enabled"
                : "disabled"
        }\``;
...
async function toggleSettingSaveButton(interaction) {
    try {
        ...
        state.serverSettings[serverId].settingsSaveButton =
            !state.serverSettings[serverId].settingsSaveButton;
        const statusMessage = `Server-wide "Settings and Save Button" is now \`${
            state.serverSettings[serverId].settingsSaveButton
                ? "enabled"
                : "disabled"
        }\``;
        ...
        console.log(
            "Error toggling server-wide settings save button:",
            error.message
        );
    }
}
...
async function downloadServerConversation(interaction) {
    try {
        ...
        const tempFileName = path.join(
            TEMP_DIR,
            `server_conversation_${interaction.id}.txt`
        );
        await fs.writeFile(tempFileName, conversationText, "utf8");
        ...
        try {
            ...
            const failDMEmbed = new EmbedBuilder()
                ...
                .setTitle("Delivery Failed")
                .setDescription(
                    "Failed to send the server-wide conversation history to your DMs."
                );
            await interaction.reply({
                embeds: [failDMEmbed],
                files: [file],
...
async function toggleServerPreference(interaction) {
    try {
        ...
        const embed = new EmbedBuilder()
            ...
            .setDescription(
                `Server response style updated to: ${state.serverSettings[guildId].responseStyle}`
            );
...
async function showSettings(interaction, edit = false) {
    try {
        // Pastikan command hanya bisa dipakai di server
        if (!interaction.guild) {
            const embed = new EmbedBuilder()
                .setColor(0xff0000)
                .setTitle("Server Command Only")
        ...
        // kalau lolos semua check, tampilkan settings
        const mainButtons = [
            {
                customId: "clear-memory",
                label: "Clear Memory",
...
async function handleSubButtonInteraction(interaction, update = false) {
    ...
    if (update || subButtonConfigs[interaction.customId]) {
        const subButtons = subButtonConfigs[
            update ? "general-settings" : interaction.customId
        ].map((config) =>
            new ButtonBuilder()
                .setCustomId(config.customId)
                .setLabel(config.label)
        ...
        while (subButtons.length > 0) {
            actionRows.push(
                new ActionRowBuilder().addComponents(subButtons.splice(0, 5))
            );
        }
            ...
            if (interaction.user.id === originalMessage.author.id) {
                try {
                    const embed = new EmbedBuilder()
                        ...
                        .setDescription(
                            "Response generation stopped by the user."
                        );
        ...
        try {
            ...
            if (isLargeResponse) {
                ...
                const shouldAddDownloadButton = originalMessage.guild
                    ? state.serverSettings[originalMessage.guild.id]
                          ?.settingsSaveButton
                    : true;
                if (shouldAddDownloadButton) {
                    botMessage = await addDownloadButton(botMessage);
                    botMessage = await addDeleteButton(
                        botMessage,
                        botMessage.id
                    );
                } else {
                ...
                const errorMsg = await originalMessage.channel.send({
                    ...
                    embeds: [
                        new EmbedBuilder()
                            ...
                            .setDescription(
                                `Generation Attempt(s) Failed, Retrying..\n\`\`\`${error.message}\`\`\``
                            ),
                    ],
                });
...
function updateEmbed(botMessage, finalResponse, message, functionCallsString) {
    try {
        ...
        console.error(
            "An error occurred while updating the embed:",
            error.message
        );
    }
}
